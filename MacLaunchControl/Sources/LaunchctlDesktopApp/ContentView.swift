        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            GroupBox(vm.editingTimerPath.isEmpty ? "Новая задача / обновление" : "Редактирование таймера") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Picker("Режим", selection: $vm.scheduleMode) {
                    ...
                    if vm.scheduleMode == .calendar {
                        ...
                        HStack {
                            Text("Интервал")
                                .frame(width: 120, alignment: .leading)
                            Stepper(value: $vm.intervalValue, in: 1...10000) {
                                Text("Каждые \(vm.intervalValue)")
                            }
                            Picker("Единица", selection: $vm.intervalUnit) {
